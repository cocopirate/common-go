...
func (r *Reporter[T, PT]) Reset(ctx context.Context) error {
	...
	return r.q.Model(ctx).Where("id = ?", r.taskID).
...
func (q *Queue[T, PT]) SetProgress(ctx context.Context, id uuid.UUID, done int) error {
	return q.Model(ctx).Where("id = ?", id).
		Updates(map[string]any{"done_count": done, "updated_at": time.Now()}).Error
}
...
func (q *Queue[T, PT]) SetTotal(ctx context.Context, id uuid.UUID, total int) error {
	return q.Model(ctx).Where("id = ?", id).Updates(map[string]any{
		"total_count": total, "done_count": 0, "updated_at": time.Now(),
	}).Error
}
